"""
Batch render clip_03 and clip_04 of Video 33 with:
  • Hook banner in subtitle zone (48-65% height), golden yellow text, pale yellow box border, black stroke outline
  • Logo: branding/logo.png, top-right, 24% width
  • Music: "TIMELESS.mp3" at 8% volume starting at 132s for both clips
"""
...
LOGO        = HERE / "logo.png"
MUSIC       = ROOT / "music" / "Tí Quang - Kamin (Instrumental) - (320 Kbps).mp3"
FONT        = ROOT / "assets" / "fonts" / "BarlowCondensed-Black.ttf"
JOB_DIR     = ROOT / "outputs" / "Video 33 - Robert Downey Jr. On Living With Intention, Discip"

# Settings for the two clips
CLIPS_CFG = {
    "clip_03": {
        "captioned": JOB_DIR / "clips" / "clip_03_captioned_barlow_black.mp4",
        "raw": JOB_DIR / "clips" / "clip_03_raw.mp4",
        "output": JOB_DIR / "clips" / "clip_03_branded.mp4",
        "hook_text": "TONY STARK THOUGHT HE HAD UNLIMITED RESILIENCE",
        "music_ss": 132.0,
    },
    "clip_04": {
        "captioned": JOB_DIR / "clips" / "clip_04_captioned_barlow_black.mp4",
        "raw": JOB_DIR / "clips" / "clip_04_raw.mp4",
        "output": JOB_DIR / "clips" / "clip_04_branded.mp4",
        "hook_text": "TONY STARK: ONE MISTAKE WILL BLOW UP YOUR LIFE",
        "music_ss": 132.0,
    }
}
...
def render_clip(clip_name, cfg):
    print(f"\n========================================\nProcessing {clip_name}...\n========================================")
    captioned = cfg["captioned"]
    raw = cfg["raw"]
    output = cfg["output"]
    hook_text = cfg["hook_text"]
    music_ss = cfg["music_ss"]

    if not captioned.exists() or not raw.exists():
        print(f"Error: files for {clip_name} not found! Skipping.")
        return
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print(f"Running ffmpeg for {clip_name}...")
        result = subprocess.run(
            cmd, capture_output=True, text=True,
            creationflags=_NO_WIN, timeout=400,
        )
        if result.returncode == 0 and output.exists():
            print(f"Success rendering {clip_name}!")
        else:
            print(f"Failed rendering {clip_name}!")
            print(result.stderr[-2000:])

def main():
    for clip_name, cfg in CLIPS_CFG.items():
        try:
            render_clip(clip_name, cfg)
        except Exception as e:
            print(f"Error rendering {clip_name}: {e}")
