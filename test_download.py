# ====================================================================
# test_download.py — Integrated YouTube Video Downloader
# ====================================================================
# Now acts as a module for the pipeline, replacing pipeline/downloader.py
# ====================================================================
...
def download_video(job_dir: str, url: str, logger: logging.Logger, progress_callback=None) -> dict:
    ...
    try:
        # Download best video and audio streams in parallel
        import threading
        logger.info("  → Downloading video (up to 4K) and audio streams in parallel...")
        if progress_callback:
            progress_callback(0, "downloading video and audio streams")
        ...
        def run_video_dl():
            nonlocal video_pct, dl_error
            try:
                def cb(pct, phase):
                    nonlocal video_pct
                    with dl_lock:
                        video_pct = pct
                        unified = int((video_pct * 0.8) + (audio_pct * 0.2))
                        if progress_callback:
                            try:
                                progress_callback(int(unified * 0.75), f"downloading video ({video_pct}%)")
                            except Exception:
                                pass
                # Look for project root cookies file
                project_root = Path(__file__).resolve().parent
                cookies = None
                for candidate in [project_root / "cookies.txt", Path.cwd() / "cookies.txt"]:
                    if candidate.is_file():
                ...
                if cookies:
                    cmd += ["--cookies", cookies]
                _run_cmd_with_progress(cmd, logger, "download", progress_callback=cb)
            except Exception as e:
                with dl_lock:
                    dl_error = e

        def run_audio_dl():
            nonlocal audio_pct, dl_error
            try:
                def cb(pct, phase):
                    nonlocal audio_pct
                    with dl_lock:
                        ...
                        if progress_callback:
                            try:
                                progress_callback(int(unified * 0.75), f"downloading audio ({audio_pct}%)")
                            except Exception:
                                pass
                project_root = Path(__file__).resolve().parent
                cookies = None
        ...
        t_video = threading.Thread(target=run_video_dl, daemon=True)
        t_audio = threading.Thread(target=run_audio_dl, daemon=True)
        t_video.start()
        t_audio.start()
        t_video.join()
        t_audio.join()
        ...
        logger.info("  ✓ Video and audio downloaded")
        ...
        # Show merge progress
        def merge_progress(pct, phase):
            if progress_callback:
                # Map merge progress from 77% to 85%
                unified_pct = 77 + int(pct * 0.08)
                try:
                    progress_callback(unified_pct, f"merging ({pct}%)")
                except Exception:
                    pass

        try:
            _run_cmd_with_progress(merge_cmd, logger, "transcode", merge_progress, duration)
...
def download_video_stream_only(
    ...
    # download — both used to write audio.m4a to the same path, racing each
    # other and leaving the bg thread to die with "audio.m4a not found" once
    # the foreground transcoded+deleted its copy. Separate workspace = no race.
    ...
    try:
        # Download best video and audio streams in parallel
        import threading
        logger.info("[VIDEO-DL] Downloading best video (up to 4K) and audio streams in parallel...")
        if progress_callback:
            ...
            try:
                def cb(pct, phase):
                    ...
                    with dl_lock:
                        ...
                        if progress_callback:
                            try:
                                progress_callback(int(unified * 0.70), "download")
                            except Exception:
                                pass
                project_root = Path(__file__).resolve().parent
                cookies = None
        ...
        logger.info("[VIDEO-DL] Video and audio streams downloaded in parallel")
        ...
        # Use progress callback for merge with actual progress tracking
        def merge_progress(pct, phase):
            if progress_callback:
                # Map merge progress from 72% to 75%
                unified_pct = 72 + int(pct * 0.03)
                try:
                    progress_callback(unified_pct, f"merge ({pct}%)")
                except Exception:
                    pass
        ...
        try:
            ...
            logger.error(f"[VIDEO-DL] Merge failed: {e}")
            raise
